...
*/
include("configurations/conf.Facebook.php");
class FacebookPlugin{
	...
	public function __construct(){
		...
		$this->application_secret=$GLOBALS['application_secret'];
		$this->connect_url="http://www.facebook.com/login.php?api_key=".$GLOBALS['api_key'];
		$this->generate_token_url="www.facebook.com/code_gen.php?v=1.0&api_key=".$GLOBALS['api_key'];
	}
		
	private function do_facebook_request($parameters, $method)
	{		
		...
		$response = $this->do_post_request('http://api.facebook.com/restserver.php', $data);
	...
	* @param string $token The token generated by user and obtained via form in callback file
	* @return string session_id To be permanently stored
	* */
	
	public function getSessionID($token){
		$xmlstr=$this->do_facebook_request(array('auth_token' => $token), 'facebook.auth.getSession');
